...
iterations with the {\it Genseset.intersectSelection} function. In order to
perforn the functions of the IntClust package for the pathway analysis, the MLP
pacakge (\citealp{Raghavan2013}) needs to be loaded first.
<<Pathways,eval=FALSE>>= 
library(MLP)
data(GeneInfo)
data(GS)
